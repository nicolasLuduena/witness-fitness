...
async function cmdAuth() {
  ...
  console.log(`authorized athlete id=${tokens.athlete.id} ${tokens.athlete.firstname} ${tokens.athlete.lastname}`)
...
async function cmdVerify(path?: string) {
  const files = path
    ? [path]
    : readdirFixtures().filter((f) => f.endsWith('.json')).map((f) => join(FIXTURES_DIR, f))
  if (files.length === 0) {
