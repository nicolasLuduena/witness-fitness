...
for (const target of targets) {
  ...
  const path = await saveFixture(result, { url: target.url, label: target.label }, "public-api");
  console.log("saved fixture:", path);
  await verifyFixture(
    JSON.parse(await import("node:fs").then((fs) => fs.promises.readFile(path, "utf-8"))),
  );
