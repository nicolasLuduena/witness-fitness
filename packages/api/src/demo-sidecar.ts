...
export const loadSidecarConfig = (env: NodeJS.ProcessEnv = process.env): DemoSidecarConfig => {
  ...
  return {
    ...
    contractAddress,
    notaryUrls: (env.NOTARY_URLS ?? 'http://127.0.0.1:8101,http://127.0.0.1:8102,http://127.0.0.1:8103')
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean),
...
export interface SidecarDeps {
  ...
  flows: {
    attest(athlete: Athlete, attestation: NotarizedAttestation, commitRand: Uint8Array): Promise<{ vaultKey: Uint8Array; tx: unknown }>;
    createWager(
      athlete: Athlete,
      input: {
    ...
    submitWorkout(athlete: Athlete, wagerId: bigint, vaultKey: Uint8Array, value: bigint): Promise<unknown>;
...
export const createDemoSidecarWithDeps = (
  ...
  const buildIdentity = async (walletCtx: WalletContext): Promise<{
    payout: Uint8Array;
    coinKey: { bytes: Uint8Array };
          ...
          deps = {
            ...
            stagePrivateState: async (athlete, fields) => {
              ...
              const base: PrivateState =
                existing ?? StrideContract.freshPrivateState(new Uint8Array(32), athleteSecret(athlete));
            ...
            identities: {
              A: { holderBinding, routing: { payout: identityA.payout, coinKey: identityA.coinKey } },
              B: { holderBinding: holderBindingB, routing: { payout: identityB.payout, coinKey: identityB.coinKey } },
            },
            holderSecret,
            holderBinding,
            verifierBinding,
  ...
  const handleAttest = async (body: unknown): Promise<{ status: number; json: unknown }> => {
    ...
    if (artifacts === undefined) {
      return { status: 400, json: { error: 'body must be { artifacts: ProofArtifacts, athlete? }' } };
    }
  ...
  const handleBadgeMint = async (body: unknown): Promise<{ status: number; json: unknown }> => {
    ...
    if (badgeId !== 1n && badgeId !== 2n) {
      return { status: 400, json: { error: 'badgeId must be 1 (streak>=3) or 2 (distance>=10000)' } };
    }
    const credential = vault.get(vaultKeyHex);
    if (credential === undefined) {
      return { status: 404, json: { error: 'unknown credential — attest first' } };
  ...
  const handleBadgeProve = async (body: unknown): Promise<{ status: number; json: unknown }> => {
    ...
    const { flows, readState } = requireReady();
    const state = await withTimeout(readState(), config.txTimeoutMs, 'readState');
    if (!state.badges.member(holderBinding) || !state.badges.lookup(holderBinding).member(badgeId)) {
  ...
  const handleWagerCreate = async (body: unknown): Promise<{ status: number; json: unknown }> => {
    ...
    wagers.put({ id: wagerId, challenger: athlete, opponent, metricId, stake, deadlineBlock, submissions: {} });
    return {
      status: 200,
      json: {
  ...
  const handleWagerSubmit = async (body: unknown): Promise<{ status: number; json: unknown }> => {
    ...
    if (credential === undefined) {
      return { status: 404, json: { error: `no credential for athlete ${athlete} — attest first` } };
    }
  ...
  const handleWagers = async (): Promise<{ status: number; json: unknown }> => {
    ...
    const bindingOf = (binding: bigint): Athlete | null =>
      binding === identities.A.holderBinding ? 'A' : binding === identities.B.holderBinding ? 'B' : null;
  ...
  const handleState = async (athlete: Athlete): Promise<{ status: number; json: unknown }> => {
    ...
    const badgeEntries = state.badges.member(binding) ? [...state.badges.lookup(binding)].map(hexOf) : [];
    return {
      status: 200,
      json: {
        athlete,
        vault: vaultEntries,
        streaks: streakEntry === null ? null : { count: hexOf(streakEntry.count), lastDay: hexOf(streakEntry.lastDay) },
        badges: badgeEntries,
      },
    };
  };
