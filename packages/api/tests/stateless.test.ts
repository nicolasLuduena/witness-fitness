...
import { createDemoSidecarWithDeps, loadSidecarConfig, type DemoSidecarConfig } from '../src/demo-sidecar.js';
...
const start = async (config: DemoSidecarConfig): Promise<{ url: string; close: () => Promise<void> }> => {
...
describe('stateless surface', () => {
  ...
  it('POST /strava/refresh posts grant_type=refresh_token', async () => {
    ...
    globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
      captured = { url: String(input), body: JSON.parse(String(init?.body)) };
      return new Response(
        JSON.stringify({ access_token: 'tok-2', refresh_token: 'tok-ref-2', expires_at: 1786000100 }),
        { status: 200, headers: { 'content-type': 'application/json' } }
      );
    }) as typeof fetch;
    try {
      ...
      expect(res.status).toBe(200);
      expect(captured?.url).toBe('https://www.strava.com/oauth/token');
      expect(captured?.body).toMatchObject({ refresh_token: 'old-refresh', grant_type: 'refresh_token' });
  ...
  it('wager-openings: deposit two, GET returns both; per-wager isolation; 404 before deposit', async () => {
    ...
    const d1 = await post(url, '/wager-openings', { wagerId: 'w-1', who: 'A', value: '0x1', rand: '0x2' });
    expect(d1.status).toBe(200);
    expect(await d1.json()).toEqual({ stored: true });
    const d2 = await post(url, '/wager-openings', { wagerId: 'w-1', who: 'B', value: '0x3', rand: '0x4' });
    ...
    const invalid = await post(url, '/wager-openings', { wagerId: 'w-3', who: 'A', value: 'not-hex', rand: '0x2' });
  ...
  it('wager-openings: TTL expiry returns 404 (short TTL)', async () => {
    ...
    try {
      const d = await post(short.url, '/wager-openings', { wagerId: 'w-ttl', who: 'A', value: '0x1', rand: '0x2' });
  ...
  it('CORS covers the new endpoints (OPTIONS → 204 + headers)', async () => {
    ...
    const res = await fetch(`${url}/wager-openings/w-cors`, { headers: { origin: 'http://localhost:5173' } });
    expect(res.status).toBe(200);
