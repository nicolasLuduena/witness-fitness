...
const expectStatus = (label: string, result: { status: number; body: Record<string, unknown> }, expected: number): void => {
...
const main = async () => {
  ...
  const attA = await post('/attest', { athlete: 'A', artifacts: loadFixture('fixture-activity-19643821429-3545m.json') });
  expectStatus('attest A', attA, 200);
  const vaultKeyA = attA.body.vaultKey as string;

  console.log('[e2e] attest B (2426 m fixture)...');
  const attB = await post('/attest', { athlete: 'B', artifacts: loadFixture('fixture-activity-19643822226-2426m.json') });
  ...
  console.log(`[e2e] create wager (A challenger, stake 10 NIGHT, deadline now+90s = ${deadline})...`);
