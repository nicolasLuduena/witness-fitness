...
import { createPrivateState, type PrivateState, type SchnorrSignature } from '@witnessfitness/contract';
...
describe('signature parity roundtrip (notary pipeline → simulator)', () => {
  ...
  it('rejects 1-of-3', async () => {
    const sim = new StrideSim(ps());
    register(sim);
    const state = await attestationFor('fixture-github-attestor-core-fresh-x-84m.json', [0, null, null]);
  ...
  it('rejects a signature from an unregistered key', async () => {
    ...
    expect(() => sim.call('verifyAttestation', ps({ assertion, signatures, holderSecret }))).toThrow(
  ...
  it('rejects replay of the same nonce', async () => {
    const sim = new StrideSim(ps());
    register(sim);
    const state = await attestationFor('fixture-github-attestor-core-fresh-x-84m.json', [0, 1, null]);
    sim.call('verifyAttestation', state);
