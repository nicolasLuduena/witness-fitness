...
describe('assert', () => {
  ...
  it('produces distinct assertions for distinct artifacts', async () => {
    const gh = await verifyReclaimProof(loadFixture('fixture-github-attestor-core-x-0m.json'), ALLOWED);
    const cg = await verifyReclaimProof(loadFixture('fixture-github-attestor-core-fresh-x-84m.json'), ALLOWED);
  ...
  it('pads claims to 8 and carries the real claimCount', async () => {
    ...
    const padding = (assertion as A_Assertion).claims.slice(Number((assertion as A_Assertion).claimCount));
