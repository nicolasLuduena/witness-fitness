...
const TEST_UNSHIELDED = 'mn_addr_undeployed1qurswpc8qurswpc8qurswpc8qurswpc8qurswpc8qurswpc8qursn4yfte';
...
describe('strava oauth surface (wallet mode)', () => {
  ...
  it('connectStrava() opens the Strava authorize URL with the callback path', async () => {
    ...
    expect(url).toContain('redirect_uri=' + encodeURIComponent('http://localhost:5173/strava/callback'));
  ...
  it('handleStravaRedirect() exchanges the code and persists the identity', async () => {
    windowShim.location.href = 'http://localhost:5173/strava/callback?code=oauth-code-123&scope=read';
    ...
    expect(client.stravaStatus!()).toEqual({ connected: true, athleteName: 'Grace Hopper', stravaId: 7 });
...
describe('two-browser wager flow (create → accept → submit → relay → settle)', () => {
  ...
  it('runs the full duel: create → accept → seal both → relay → settle reveal', async () => {
    ...
    expect(relayed.every((o) => /^0x[0-9a-f]+$/.test(o.value) && /^0x[0-9a-f]+$/.test(o.rand))).toBe(true);
  ...
  it('settles as a forfeit when the opponent never submits (their opening never relays)', async () => {
    ...
    const created = await clientA.createWager({
      opponent: {
        ...
        handle: 'opponent',
        role: 'opponent',
        holderBinding: sessionA.athlete.holderBinding === '0x0' ? '0x1' : sessionB.athlete.holderBinding,
      },
      metricId: 1n,
      stake: 1,
      deadlineBlock: BigInt(Math.floor(Date.now() / 1000) + 90),
