...
type RouteHandler = (body?: Record<string, unknown>) => {
  status: number;
...
const installStubSidecar = (): { records: Map<bigint, StubWager> } => {
  ...
  routes.set("/wager/accept", (body) => {
    ...
    return {
      status: 200,
      json: { id: HEX(id), athlete: body?.athlete, accepted: true, txHash: HEX(0xdef) },
    };
  });
  ...
  routes.set("/wager/settle", (body) => {
    ...
    return {
      ...
      json: {
        ...
        nft: record.winner === "tie" ? null : { tokenType: "0x00f00d", txHash: HEX(0x777) },
