...
const createConnectedStub = (opts: StubWalletOptions): ConnectedAPI =>
  ({
    ...
    getConnectionStatus: async () => ({ status: 'connected', networkId: opts.networkId ?? 'undeployed' }),
...
describe('wallet bridge stub', () => {
  ...
  it('export/import roundtrip preserves the private state; wrong password fails', async () => {
    ...
    await expect(bridge3.importPrivateState('wrong', 'wf-test-store', payload)).rejects.toThrow(/wrong backup password/);
  });
  ...
  it('reset clears the stored private state', async () => {
    ...
    await expect(bridge.exportPrivateState('pw', 'wf-test-store')).rejects.toThrow(/no private state/);
  });
});
...
describe('wallet restore/resume helpers', () => {
  ...
  it('auto-resume prompts only when a backup exists, no live session, not prompted yet', () => {
    expect(shouldAutoResume({ hasBackup: true, hasCredentials: false, alreadyPrompted: false })).toBe(true);
    expect(shouldAutoResume({ hasBackup: false, hasCredentials: false, alreadyPrompted: false })).toBe(false);
    expect(shouldAutoResume({ hasBackup: true, hasCredentials: true, alreadyPrompted: false })).toBe(false);
    expect(shouldAutoResume({ hasBackup: true, hasCredentials: false, alreadyPrompted: true })).toBe(false);
  });
});
