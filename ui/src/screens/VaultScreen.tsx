    ...
    <div className="screen">
      <div className="screen-header">
        ...
        <p className="screen-sub">
          Each entry is a <code className="mono">persistentCommit</code> bound to your holder secret.
          The chain knows <em>what you can prove</em> — never what it was. Hover an envelope.
        </p>
      </div>

      {credentials.length === 0 ? (
        <div className="empty-state">
          No credentials yet. Attest a workout on the Connect tab{session ? '' : ' (enter the demo first)'}.
        </div>
      ) : (
            ...
            return (
              <Card key={cred.id} title={isNewest ? 'Newest attestation' : 'Vaulted credential'}>
                ...
                <div className="row-between faint" style={{ fontSize: 12.5, marginTop: 4 }}>
                  ...
                  <span>{cred.source === 'demo-story' ? 'demo session' : 'attested-session replay'}</span>
                </div>
              </Card>
            );
          })}
